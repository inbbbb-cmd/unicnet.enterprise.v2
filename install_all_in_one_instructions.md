...
3. **Настройка переменных окружения**:
   ...
   1. **Отредактируйте файл `export_variables.txt`** в корне репозитория:
      ...
      > - `RouterCidr` - **ОБЯЗАТЕЛЬНО** замените на CIDR локальной сети (для NAT — внутренний RFC1918 CIDR, не WAN)
      >
      > Для окружения за NAT:
      > - указывайте только локальные подсети, которые должен обслуживать router;
      > - при нескольких подсетях задавайте список через запятую без пробелов (пример: `10.10.0.0/16,192.168.100.0/24`);
      > - проверьте значение через `ip route`/`ip addr`, чтобы маска соответствовала реальной сети.
